...
window.onload = function init() {
    ...
    var projectionMatrix = perspective(60, canvas.width/canvas.height, 1, 500);
    ...
    var lightPos = vec4(1.0, 1.0, 2.0, 0.0);
...
function lightBulb() {
    ...
    var matrix = translate(8.0, 7.5, -15);
    drawSphere(matrix);
}
...
function resetModelViewMatrix() {
    modelViewMatrix = [mult(translate(0, 0, -25), rotate(theta.body, 0, 1, 0))];
}
...
function body() {
    ...
    var t = mult(currentMatrix(), s);

    draw(t, 1, 1);
}

function head() {
    var newModelViewMatrix = mult(currentMatrix(), translate(-0.25 * size.body[0] , 0.7 * size.body[1], -0.25 * size.body[2]));
    ...
    instanceMatrix = mult(s, instanceMatrix)
